...
The script:
1. assumes **AWSTransform-Infra-Deployment-Role** 
...
The script assumes the **AWSTransform-Infra-Deployment-Role** by default for proper permissions (use `-SkipAssumeRole` to skip this). 

If using a different role than the default `AWSTransform-Infra-Deployment-Role` you need to:
...
2. **assumes the AWSTransform-Application-Deployment-Role** by default for deployment permissions
...
### Using a different IAM Role
The script assumes the **AWSTransform-Application-Deployment-Role** by default for proper permissions.
